...
{
    public class TaskTypeAPIHelper
    {
        #region Initialization

        private HttpClient httpClient;

        public TaskTypeAPIHelper()
        {
        ...
        public async Task<HttpResponseMessage> PostType(string token, TypeModel typeData)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await httpClient.PostAsJsonAsync("/api/TaskType", typeData))
                {
                    return response;
                }
            }
        ...
        public async Task<ObservableCollection<TypeModel>> GetTypes(string token)
        {
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                httpClient.DefaultRequestHeaders.Add("Authorization", $"bearer {token}");
                using (HttpResponseMessage response = await httpClient.GetAsync("/api/TaskType"))
                {
        ...
        public async Task<HttpResponseMessage> DeleteType(string token, long id)
        {
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                httpClient.DefaultRequestHeaders.Add("Authorization", $"bearer {token}");
                using (HttpResponseMessage response = await httpClient.DeleteAsync($"/api/TaskType/{id}"))
                {
                    return response;
                }
            }
        ...
        public async Task<HttpResponseMessage> PutType(string token, TypeModel typeData)
        {
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                httpClient.DefaultRequestHeaders.Add("Authorization", $"bearer {token}");
                using (HttpResponseMessage response = await httpClient.PutAsJsonAsync("/api/TaskType", typeData))
                {
                    return response;
                }
            }
