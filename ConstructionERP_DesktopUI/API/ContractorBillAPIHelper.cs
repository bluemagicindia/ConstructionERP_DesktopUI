...
namespace ConstructionERP_DesktopUI.API
{
    public class ContractorBillAPIHelper
    {
        ...
        #region Put Contractor Bill

        public async Task<HttpResponseMessage> PutContractorBill(string token, ContractorBillModel billData)
        {
            try
            {
                httpClient.DefaultRequestHeaders.Clear();
                ...
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                httpClient.DefaultRequestHeaders.Add("Authorization", $"bearer {token}");
                using (HttpResponseMessage response = await httpClient.PutAsJsonAsync("/api/ContractorBills", billData))
                {
                    return response;
                }
            }
